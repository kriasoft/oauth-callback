...
- Credential invalidation (`"all"`, `"client"`, `"tokens"`) wins over work already in flight, without relying on attempt identity (the SDK gives hooks none, and transports the caller creates share one owner). A token exchange checks the invalidation generation it took when it started; any other token save is a refresh and may only replace tokens still stored, so it can't restore invalidated ones. A registration checks the generation its auth pass saw when reading `clientInformation()`.
