...
- Credential invalidation (`"all"`, `"client"`, `"tokens"`) wins over work already in flight. It bumps a session generation; each SDK auth pass is stamped when it reads `clientInformation()` (and a token exchange when it starts), and a registration, refresh or exchange saved under a stale stamp is refused, so a late response can't undo a sign-out.
- Discovery state is pinned to the active flow like its verifier and client, so an overlapping attempt's discovery can't reach its callback leg.
