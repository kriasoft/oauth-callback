...
- Every authorization URL is checked before any launcher runs: `https:` (or loopback `http:`), no fragment or credentials, interpreted parameters at most once, `response_type` absent or `code`, `response_mode` absent or `query`. A prebuilt PAR/JAR URL is refused (outer parameters aren't authoritative, RFC 9126 §2.1, RFC 9101 §6.3).
